...
SELECT users.user_name, hotels.rating, SUM(bookings.total_price) total_spent
FROM users
JOIN bookings ON bookings.user_id = users.user_id
JOIN hotels ON hotels.hotel_id = bookings.hotel_id
WHERE bookings.status = 'COMPLETED'
GROUP BY hotels.rating, users.user_id
HAVING total_spent > 5000000
ORDER BY hotels.rating DESC, total_spent DESC;